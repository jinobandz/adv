// api/db.js — Brokie's Auto Services
...
module.exports = async function handler(req, res) {
  ...
  try {
    switch (action) {
      ...
      case 'get_accounts': {
        const accounts = await get('ba_accounts') || {};
      ...
      case 'set_account': {
        ...
        await set('ba_accounts', accounts);
      ...
      case 'push_event': {
        const events = await get('ba_events') || [];
        events.unshift({ ...payload.event, ts: Date.now() });
        await set('ba_events', events.slice(0, 500));
        return res.json({ ok: true });
      }
      case 'get_events': {
        const events = await get('ba_events') || [];
      ...
      case 'log_token': {
        ...
        const tokens = await get('ba_tokens') || [];
        if (!tokens.find(t => t.token === token)) {
          tokens.unshift({ token, label, ts: Date.now() });
          await set('ba_tokens', tokens.slice(0, 100));
        }
        return res.json({ ok: true });
      }
      case 'get_tokens': {
