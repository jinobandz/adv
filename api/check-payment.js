...
async function checkETH(address, expectedAmount, since) {
  try {
    // Uses Etherscan free tier (no API key needed for basic queries up to 5 req/s)
    // Set ETHERSCAN_API_KEY env var in Vercel for higher limits
    const apiKey = process.env.ETHERSCAN_API_KEY || '';
    const keyParam = apiKey ? `&apikey=${apiKey}` : '';
    const res = await fetch(`https://api.etherscan.io/api?module=account&action=txlist&address=${address}&startblock=0&endblock=99999999&sort=desc${keyParam}`);
    const data = await res.json();
    ...
    return filteredTxs.some(tx => parseInt(tx.value) >= expected * 0.99);
  } catch { return false; }
}
